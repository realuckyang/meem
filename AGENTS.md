...
- `worker/ui/meem/`：Meem 控制台前端（React + Vite + TS），对应根路径 `/`。
- `worker/ui/meem/src/apps/`：Meem 内应用，聊天、终端、文件、状态、截图都在这里，对应 `/apps/:appname`。
- `worker/ui/meem/src/system/`：Meem 前端系统层，负责全局顶栏、应用面板、路由和共享 client。
...
```bash
# Frontend
npm --prefix worker/ui/meem install
npm --prefix worker/ui/meem run check
npm --prefix worker run build:ui
