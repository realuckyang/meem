...
const parseMultipartUpload = async (req, targetDir) => {
  ...
  if (!m) throw new Error('missing_multipart_boundary')
...
export const handleFsApi = async (req, res, pathname, url) => {
  ...
  try {
    ...
    if (pathname === '/api/fs/stat' && req.method === 'GET') {
      const p = String(url.searchParams.get('path') || '')
      if (!p) return fail(res, 'path_required', 400)
    ...
    if (pathname === '/api/fs/read' && req.method === 'GET') {
      ...
      if (!p) return fail(res, 'path_required', 400)
      const st = await fsp.stat(p)
      if (st.isDirectory()) return fail(res, 'is_directory', 400)
    ...
    if (pathname === '/api/fs' && req.method === 'DELETE') {
      ...
      const st = await fsp.stat(p)
      if (st.isDirectory()) {
        if (!recursive) return fail(res, 'need_recursive', 400)
    ...
    if (pathname === '/api/fs/rename' && req.method === 'POST') {
      ...
      if (!from || !to) return fail(res, 'from_to_required', 400)
      await fsp.rename(from, to)
      return ok(res, { from, to })
    }
    if (pathname === '/api/fs/upload' && req.method === 'POST') {
      // 接受相对路径(按 process.cwd() 解析)或绝对路径;返回绝对路径
    ...
    return fail(res, err?.message || 'fs_error', 500)
  }
}
