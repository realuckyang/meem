...
export function makeDevices(env: Env, uid: string): DevicesRepo {
  ...
  return {
    async listDevices() {
      return (await DB.prepare(`SELECT ${COLS} FROM meem_devices WHERE meem_uid=? ORDER BY kind, created`).bind(uid).all<DeviceRow>()).results;
    },
    async getDevice(id) {
      return DB.prepare(`SELECT ${COLS} FROM meem_devices WHERE id=? AND meem_uid=?`).bind(id, uid).first<DeviceRow>();
    },
    async createDevice(p) {
      const id = uuid();
      const token = genToken();
      await DB.prepare(`INSERT INTO meem_devices (${COLS}) VALUES (?,?,?,?,?,?,?,?,?)`)
    ...
    async updateDevice(id, p) {
      ...
      await DB.prepare(`UPDATE meem_devices SET ${cols.join(',')} WHERE id=? AND meem_uid=?`).bind(...vals).run();
    },
    async deleteDevice(id) {
      await DB.prepare('DELETE FROM meem_devices WHERE id=? AND meem_uid=?').bind(id, uid).run();
    },
  };
}
...
export async function verifyDevice(env: Env, token: string): Promise<{ id: string; uid: string; kind: string } | null> {
  ...
  const row = await env.DB.prepare("SELECT id,meem_uid,kind FROM meem_devices WHERE token=? AND status='active'").bind(token).first<{ id: string; meem_uid: string; kind: string }>();
