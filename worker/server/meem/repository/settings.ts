...
export function makeSettings(env: Env, uid: string): SettingsRepo {
  const DB = env.DB;
  const ensure = () => DB.prepare('INSERT OR IGNORE INTO meem_settings (meem_uid) VALUES (?)').bind(uid).run();
  return {
    async loadSettings() {
      await ensure();
      const s = await DB.prepare('SELECT llm_url,llm_key,llm_model,max_rounds,persona,vision FROM meem_settings WHERE meem_uid=?').bind(uid).first<SettingsLite>();
      return s ?? { llm_url: '', llm_key: '', llm_model: '', max_rounds: 30, persona: '', vision: 0 };
    },
    async getSettings() { await ensure(); return (await DB.prepare('SELECT * FROM meem_settings WHERE meem_uid=?').bind(uid).first()) ?? {}; },
