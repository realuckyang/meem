...
users
  settings.user_id
  contacts.user_id
  inbox_threads.user_id
  inbox_messages.user_id
  sessions.user_id
  session_events.user_id
```

这让一个 Worker 可以承载多个本地用户，同时每个用户拥有独立的公开地址、通讯录、收件箱和智能体会话。
...
- 公开地址、收件箱、会话、事件必须始终按 `user_id` 隔离。
